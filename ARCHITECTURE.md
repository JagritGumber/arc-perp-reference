...
| Version | Scope | Depends on | Est. LOC (delta) | Risk / complexity |
|---|---|---|---|---|
| **v0.1** *(shipping today)* | Interfaces + `OrderTypes` library + `AccountManager` impl + `USDCVault` impl (deposit/withdraw + margin-hook scaffold) + Deploy wiring AccountManager+USDCVault + ADRs 0001/0002 + AccountManager unit/fuzz tests + USDCVault unit/fuzz tests + integration test (register → deposit → withdraw round-trip) | — | ~1,100 Sol + 700 test | Low. Two primitives, both production-shaped |
...
| **v0.3 — MarketRegistry** | `MarketRegistry.sol` with admin curation, Pyth adapter interface, paused-toggle, markPrice view, governance multisig wiring | v0.1 | ~250 Sol + 200 test | Low–Med. Pyth integration is the only unknown |
